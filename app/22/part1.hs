...
safeDisintegrate :: Set Brick -> Int -> Bool
safeDisintegrate bricks brickNo = withoutBrickNo == collapsed
  where
    withoutBrickNo = Set.filter (\(_, _, i) -> i /= brickNo) bricks
    collapsed = collapse 1 withoutBrickNo

findAnswer :: Set Brick -> Int
findAnswer bricks = Set.size safeBricks
  where
    collapsed = collapse 1 bricks
    brickNos = Set.map (\(_, _, i) -> i) bricks
    safeBricks = Set.filter (safeDisintegrate collapsed) brickNos
